...
class RNNModel(nn.Module):
    ...
    def generate_text(self, start_text, length, vocab_to_int, int_to_vocab, device):
        self.eval()  # 切換到評估模式
        input_seq = torch.tensor([vocab_to_int[c] for c in start_text], dtype=torch.long).unsqueeze(0).to(device)
        hidden = self.init_hidden(1, device)
        generated_text = start_text

        for _ in range(length):
            output, hidden = self(input_seq, hidden)
            output = output[:, -1, :]  # 取得最後一個時間步的輸出
            probs = torch.softmax(output, dim=-1).cpu().numpy().flatten()
            next_char_idx = np.random.choice(len(probs), p=probs)  # 根據機率選擇下一個字符
...
if __name__ == "__main__":
    ...
    model, history = RNNModel(vocab_size, embedding_dim, hidden_dim).to(device)
    # 設定起始詞和生成參數
    start_text = "JULIET"
